...
console.info("WebCorrector App Version: %s", ver);
...
var toolbarModel = Backbone.View.extend({
    ...
    render: function(){
      ...
      var self = this;
      dust.render(this.TemplateKey, {}, function(err, out) {
      ...
      $('.wctwrap > li').on('click', function(e) {
          ...
          var open = webC.helper.addDraw(color);
            ...
            */
            ...
            //     autoopen: true
            // });
            if (pagescore > 0)  { $('.wcscore', that).fadeIn(); that.popover('title',that.data('ptitle')).popover('content',that.data('pcontent')).popover('fadeIn'); 
            ...
            webC.apis.get('validate', {
              url: 'http://'+$orgUrl
                ...
                if (data.messages.length > 0) {
                    ...
                    if (td.score == 0) {  _.extend(td, {color:'cgreen', bgcolor:'bgreen', msg:'excellent'}) }
                    else if (td.score > 0 && td.score <= 30) { _.extend(td, {color:'cred', bgcolor:'bred', msg:''}) }
                    if (td.score > 30) { _.extend(td, { msg:'toomany'}) }
                    // toolbar/htmlvalidate.dust template
                    dust.render("toolbar/htmlvalidate", td, function(err, out) {
            ...
            if (!that.is('.active')) {
            that.addClass('active');
            if (score > 0)  { $('.wcscore', that).fadeIn(); that.popover('title',that.data('ptitle')).popover('content',that.data('pcontent')).popover('fadeIn'); return }
            that.popover('hide');
            ...
            webC.apis.get('pagespeed', {
              url: 'http://'+$orgUrl
                ...
                if (data.score > 0) {
                    ...
                    var s = td.score;
                    if (s >= 75 && s <= 88) { _.extend(td, {color:'cyellow', bgcolor:'byellow', color:'yellow', speed:'ok'}) }
                    else if (s > 88 && s <= 93) { _.extend(td, {color:'cgreen',bgcolor:'bgreen', speed:'good'}) }
                    if (s > 93) _.extend(td, {speed:'excellent'});
                    // toolbar/pagespeed.dust template
                    dust.render("toolbar/pagespeed", _.extend(td, data.pageStats) , function(err, out) {
                      $('.wcscore', that).text(td.score).addClass(td.bgcolor).fadeIn();
                      that.data({'pagescore':td.score,'ptitle':td.ptitle,'pcontent':out}).popover('title',td.ptitle).popover('content',out).popover('fadeIn').spin(false);
                    });
                }
            ...
            $spellchecker.on('check.complete', function(words) {
              ...
              setTimeout(function() { 
                ...
                dust.render("toolbar/spellchecker", {errors: l} , function(err, m) {
...
var SideMenusModel= Backbone.View.extend({
    ...
    initialize: function(){
      this.snapper = {};
      this.TemplateKey = 'sidemenu/leftright';
      this.render();
    },
    rerender: function(){
    ...
    render: function(){
    var self = this;
    dust.render(this.TemplateKey, this.model.toJSON(), function(err, out) {
        ...
        //     trackerColor: "#303030"
        // });
        new wcDropDown( $('.wcdd-wrap'), function(val, data) {
...
var isInspecting, isSelected = isEdit = isHover = false;
...
addStikers: function(color, text, icon, el, width, height) {
    ...
    return $("<div />", { 
      ...
      .append($("<div />", { 
        ...
        $( this ).css( props );
          ...
          var that = this;
          setTimeout(function() { $(that).removeClass("draggin"); refresh(data.id); }, 100);           
      }).css({
      position: "absolute",
      "top" : data.top,
      "left": data.left,
  ...
  refresh = function refresh(id) {
    ...
    if (el.data('linked') && !el.is('.draggin')) {
      ...
      if (el.css('position') === 'fixed') { lbox.top -= $(window).scrollTop(); lbox.left -= $(window).scrollLeft(); }
   ...
   createMarker = function createMarker(data) {
    ...
    return $("<div />", { 
      ...
      .append($("<div />", { 
        ...
        "style" : "background-color:"+color,
        click : function(e) { if ($(this).parent().is('.draggin')) { return }; e.stopPropagation(); $(elm).trigger('click'); }
        }).append($("<span />", { 
          "class" : "close-sticky hint--right", 
          "data-hint" : $.i18n.t("del"),
          text : "x", 
          click : function (e) { e.stopPropagation(); deleteBox($(this).parents(".markeround").attr("id")); }
        }))
        .append($("<div />", {
        "class" : data.icon, 
       }))).drag("start",function( ev, dd ){
   ...
   $(window).smartresize(function() {
    ...
    return $("<div />", { 
      ...
      .append($("<span/>", {
        ...
        "data-hint" : $.i18n.t("rgravity"),
        click : function (e) { e.stopPropagation(); $(this).parent().removeData("linked").css({'position':'absolute'}); $(this).hide();}
      }))
      .append($("<span />", { 
          "class" : "wcicon selected hint--left",
          "data-hint" : $.i18n.t("sel"),
      ...
      .append($("<div />", { 
       ...
       if ( dd.attr.indexOf("dragbox") > -1 ){
        ...
        //$('.dragbox-content', this).css({'width':props.width-50,'height':props.height-38});
          ...
          var that = this;
          var s = webC.helper.addStikers();
          setTimeout(function() { $(that).removeClass("draggin"); s.refresh(data.id); }, 100);           
      }).css({
      position: "absolute",
      "top" : data.top,
      "left": data.left
...
addDraw: function(color) {
  ...
  createDrawElement = function createDrawElement(data) {
    ...
    $("<div />", { 
      ...
      'id' : data.id
        ...
        }, function(e) { if ($(this).is('.drawdrag')) return;
      ...
      .prepend($("<div />", { 
        ...
        $( this ).parent().css( props );
          ...
          var that = this;
          setTimeout(function() { $(that).parent().removeClass("drawdrag"); refresh(data.id); }, 300);   
      }).append($("<span />", { 
      ...
      .append($("<div />", {
        ...
        click: function (e) {e.preventDefault();e.stopPropagation(); },
        dblclick: function (e) {e.preventDefault();e.stopPropagation(); 
     ...
     if ( dd.attr.indexOf("dragdraw") > -1 ){
        ...
        $( this ).css( props );
          ...
          var that = this;
          setTimeout(function() { $(that).removeClass("drawdrag"); refresh(data.id)}, 300);           
      }).css({
      position: "absolute",
      "top" : data.top,
      "left": data.left
  ...
  toolSelect = function toolSelect(tool, el) {
    if (tool == 'shot') {
      ...
      if (!$(el).parent().parent().hasClass('grab')) { $(el).parent().find(".dragdraw-brush").click(); return false; }
    } else if (tool == 'brush') {
      sketch.setTool(tool);
    } else if (tool == 'eraser') {
      sketch.setTool(tool)
...
webC.cookies = {
    ...
    create: function(name,value,days) {
        if (days) { var date = new Date(); date.setTime(date.getTime()+(days*24*60*60*1000)); var expires = "; expires="+date.toUTCString();} else var expires = ""; document.cookie = name+"="+value+expires+"; path=/";
    },
    read: function(name) {
...
dust.render("report-popup", {data: 'l'} , function(err, out) {
  $(out).appendTo(document.body);
});
