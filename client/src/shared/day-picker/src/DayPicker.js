...
export class DayPicker extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      ...
      (prevProps.month !== this.props.month && !DateUtils.isSameMonth(prevProps.month, this.props.month))
    ) {
  ...
  focusNextWeek(dayNode) {
    ...
    if (isInLastWeekOfMonth) {
      this.showNextMonth(() => {
        ...
        Helpers.getDayNodes(this.dayPicker, this.props.classNames)[nextMonthDayNodeIndex].focus();
      });
    } else {
  ...
  focusPreviousWeek(dayNode) {
    ...
    if (isInFirstWeekOfMonth) {
      this.showPreviousMonth(() => {
        const previousMonthDayNodes = Helpers.getDayNodes(this.dayPicker, this.props.classNames);
  ...
  handleDayClick = (day, modifiers, e) => {
    e.persist();
    if (modifiers[this.props.classNames.outside] && this.props.enableOutsideDaysClick) {
  ...
  renderNavbar() {
    const { labels, locale, localeUtils, canChangeMonth, navbarElement, ...attributes } = this.props;
    ...
    return Helpers.isValidElement(navbarElement)
      ? cloneElement(navbarElement, props)
      : createElement(navbarElement, props);
  }
  ...
  renderFooter() {
    if (this.props.todayButton) {
      return <div className={this.props.classNames.footer}>{this.renderTodayButton()}</div>;
    }
    return null;
  }
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        >
          {this.renderNavbar()}
          <div className={this.props.classNames.months}>{this.renderMonths()}</div>
          {this.renderFooter()}
        </div>
      </div>
    );
