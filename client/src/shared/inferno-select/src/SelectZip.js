...
const Select = createClass({
	...
	componentWillReceiveProps(nextProps) {
		...
		if (nextProps.value && this.state.inputValue && nextProps.value !== this.state.inputValue) {
			this.setState({
	...
	componentDidMount() {
		if (this.props.selectedValue && this.props.selectedValue.value) this.selectValue(this.props.selectedValue);
	},
	...
	renderValue(valueArray, isOpen) {
		...
		if (this.props.multi) {
			...
			return (
				<ValueComponent id={this._instancePrefix + '-value-item'} disabled={this.props.disabled} instancePrefix={this._instancePrefix} onClick={onClick} value={valueArray[0]}>
	...
	renderMenu(options, valueArray, focusedOption) {
		if (options && options.length) {
			return this.props.menuRenderer({
				...
				isKeyPressed: this.state.isKeyPressed,
	...
	renderHiddenField(valueArray) {
		...
		if (this.props.joinValues) {
			...
			return <input type="hidden" ref={ref => (this.value = ref)} name={this.props.name} value={value} disabled={this.props.disabled} />;
		}
	...
	render() {
		...
		let options = (this._visibleOptions = this.props.options);
		let isOpen = this.state.isOpen;
		if ((this.props.multi && valueArray.length && !this.state.inputValue) || !options.length) {
			isOpen = false;
		}
		...
		let className = classNames('Select search-input', this.props.className, {
		...
		return (
			<div ref={ref => (this.wrapper = ref)} className={className} style={this.props.wrapperStyle}>
				...
				>
					...
					{this.renderLoading()}
					{this.renderClear()}
					{options.length > 0 && this.renderArrow()}
				</div>
