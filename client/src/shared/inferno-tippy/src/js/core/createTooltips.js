...
export default function createTooltips(els) {
    return els.reduce((a, el) => {
        const settings = this.settings.performance ? this.settings : getIndividualSettings(el, this.settings);
        ...
        trigger.trim().split(' ').forEach(event => (listeners = listeners.concat(createTrigger(event, el, handlers, touchHold))));
