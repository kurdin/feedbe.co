...
export default function getEventListenerHandlers(el, popper, settings) {
    const { delay, interactive, hideOnClick, trigger, touchHold } = settings;
    ...
    const show = event => (this.callbacks.wait ? this.callbacks.wait.call(popper, _show, event) : _show());
    ...
    const handleTrigger = event => {
        ...
        if (event.target.className && event.target.className.indexOf('ignore-tooltip-click') > 0) return;
