...
export default class TextareaAutosize extends Component {
	...
	render() {
		...
		let maxHeight = Math.max(props.style.maxHeight || Infinity, this.state.maxHeight);
	...
	_resizeComponent = (callback = noop) => {
		...
		if (this.state.height !== height || this.state.minHeight !== minHeight || this.state.maxHeight !== maxHeight) {
