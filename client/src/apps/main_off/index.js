/*global $, location, console, localStorage */
// import { CMCode } from './editor';
...
let examples = window.datashared && window.datashared.examples;
let zip = window.datashared && window.datashared.zip;
let zipData = window.datashared && window.datashared.zipData;
// let sharedCode = window.datashared && window.datashared.sharedCode;
// let csrfToken = window.datashared && window.datashared.csrfToken;

hydrate(<AppRouter examples={examples} />, document.getElementById('app'));
hydrate(
...
$().ready(() => {
	...
	$('.parallax')
		...
		.simpleParallax({ scale: 1.3, orientation: 'bottom', transition: 'cubic-bezier(0,0,0,1)' });
});
