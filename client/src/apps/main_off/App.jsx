...
class AppMain extends Component {
	...
	constructor(props) {
		...
		const code = examples.filter(code => code.id === this.props.params.id)[0] || null;
		this.state = {
			examples: examples,
		...
		if (props.doRun) {
			let code = examples.filter(code => code.id === this.props.params.id)[0];
          ...
          <a class={ addClass({'is-primary': navSelected === 'favorites'}, 'button') } onClick={ this.on.navlist.select('favorites') }><span class="icon">&#0083;</span> <span>Favorites</span>
        	...
        	*/}
				...
				<div class="columns is-multiline">
					{examples.length > 0 ? (
						examples.map(example => <CodeCard on={this.on.card} key={example.id} copied={copied} example={example} />)
					) : (
						<div class="column">
							<div class="has-text-centered is-6">Nothing Found :( Change Your Filter</div>
							<div class="has-text-centered is-6">Nothing Found :( Change Your Filter</div>
							<div class="has-text-centered is-6">Nothing Found :( Change Your Filter</div>
							<div class="has-text-centered is-6">Nothing Found :( Change Your Filter</div>
							<div class="has-text-centered is-6">Nothing Found :( Change Your Filter</div>
