...
var lookup = function(zip) {
    if (zip === null && zip === undefined && typeof zip !== 'string') {
        return ['Expecting input Zip as String'];
    }
    let zips = [];
    Object.keys(codes.codes).forEach(key => {
        if (key.indexOf(zip.toUpperCase()) === 0) zips.push(codes.codes[key]);
    });
    return zips;
};
...
const lookupByCity = function(city) {
    if (city === null && city === undefined && typeof city !== 'string') {
        return ['Expecting input City as String'];
    }
    let cities = [];
    Object.keys(codes.codes).forEach(key => {
        const cityMDZipUpperCase = `${codes.codes[key].city.toUpperCase()} ${codes.codes[key].state.toUpperCase()} ${
            codes.codes[key].zip
        }`;
        if (cityMDZipUpperCase.indexOf(city.toUpperCase()) !== -1) cities.push(codes.codes[key]);
    });
    return cities;
};

exports.lookupCity = lookupByCity;
